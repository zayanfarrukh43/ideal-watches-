...
// 2. Added a `slug` property to generate clean collection URLs
const styles = [
  {
    id: 1,
    title: "Leather Strap",
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
          {styles.map((style, index) => (
            /* 3. Wrapped each item in a Link component pointing to /collections/:slug */
            <Link key={style.id} to={`/collections/${style.slug}`} className="block">
              <motion.div
                initial={{ opacity: 0, y: 15 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="group relative flex flex-col items-center justify-between p-6 sm:p-8 bg-zinc-950/60 border border-zinc-900 hover:border-zinc-700 transition-all duration-500 cursor-pointer h-full"
              >
